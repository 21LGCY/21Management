  ...
  return (
    <div className="flex items-center justify-between p-3 bg-dark rounded-lg border border-gray-800">
      <div className="flex items-center gap-3">
        ...
        <div>
          <p className="font-medium text-white">{player?.username || unknownPlayerLabel}</p>
          {player?.in_game_name && (
      ...
      <div className="bg-dark-card border border-gray-800 rounded-xl max-w-2xl w-full max-h-[90vh] overflow-hidden flex flex-col">
            ...
            <div>
              ...
              <div className="flex items-center gap-4 text-sm text-gray-400">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span>{responses.length} {t('responses')}</span>
                </div>
              </div>
            </div>
            <button
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <div className="bg-red-500/10 border border-red-500/30 rounded-lg p-3">
              <div className="flex items-center gap-2 text-red-400 mb-1">
                ...
                <span className="text-xs font-medium">{t('unavailable')}</span>
              </div>
            ...
            <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-lg p-3">
              <div className="flex items-center gap-2 text-yellow-400 mb-1">
                ...
                <span className="text-xs font-medium">{t('maybe')}</span>
              </div>
        ...
        {currentUserId && (
          <div className="p-6 border-b border-gray-800 bg-gray-900/30">
            <p className="text-sm font-medium text-gray-300 mb-3">{t('yourAvailability')}</p>
        ...
        <div className="flex-1 overflow-y-auto p-6">
          <h3 className="text-sm font-medium text-gray-300 mb-4">{t('playerResponses')}</h3>
          ...
          {loading ? (
            ...
            <p className="text-gray-400 text-center py-8">{t('noResponsesYet')}</p>
          ) : (
            <div className="space-y-2">
              {responses.map((response) => (
                <ResponseItem key={response.id} response={response} unknownPlayerLabel={tCommon('unknownPlayer')} />
              ))}
            </div>
          )}
        </div>

...
import { TeamCategory, TryoutStatus, ProfileTryout } from '@/lib/types/database'
import { 
  GameType, 
  GAME_CONFIGS, 
  getGameConfig,
...
import { ArrowLeft, User, Gamepad2, Link as LinkIcon, Settings, Trash2, UserPlus } from 'lucide-react'
...
const getGameFromTeamCategory = (teamCategory: TeamCategory): GameType => {
...
export default function EditScoutForm({ scoutId }: EditScoutFormProps) {
  ...
  const [teams, setTeams] = useState<Array<{ id: string; name: string; team_category: TeamCategory }>>([])
  ...
  const [originalData, setOriginalData] = useState<ProfileTryout | null>(null)
  ...
  // Update game when team changes and reset game-specific fields only if team changed
  useEffect(() => {
    const newGame = getGameFromTeamCategory(formData.team_category)
    if (newGame !== formData.game && originalData) {
      // Only reset if the user manually changed the team
      const originalGame = getGameFromTeamCategory(originalData.team_category)
      if (newGame !== originalGame) {
        setFormData(prev => ({
          ...prev,
          game: newGame,
  ...
  }, [formData.team_category, formData.game, originalData])
  ...
  const fetchScoutData = async () => {
    try {
      ...
      if (data) {
        ...
        const scoutGame = getGameFromTeamCategory(data.team_category)
        setFormData({
          ...
          faceit_level: data.faceit_level?.toString() || '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (currentGame === 'valorant') {
        updateData.valorant_tracker_url = formData.valorant_tracker_url || null
        // Clear CS2 specific fields
        updateData.faceit_level = null
        updateData.steam_url = null
        updateData.faceit_url = null
      } else if (currentGame === 'cs2') {
        updateData.faceit_level = formData.faceit_level ? parseInt(formData.faceit_level) : null
        updateData.steam_url = formData.steam_url || null
        updateData.faceit_url = formData.faceit_url || null
        // Clear Valorant specific fields
        updateData.valorant_tracker_url = null
      }
  ...
  const handlePromoteToPlayer = async () => {
    if (!confirm(tForm('confirmPromote', { name: formData.in_game_name || formData.username }))) return

    try {
      // Find the appropriate team based on team_category
      const matchingTeam = teams.find(t => t.team_category === formData.team_category)
      ...
      const { error: profileError } = await supabase
        ...
        .insert([{
          ...
          faceit_level: formData.faceit_level ? parseInt(formData.faceit_level) : null,
          steam_url: formData.steam_url || null,
          faceit_url: formData.faceit_url || null,
          twitter_url: formData.twitter_url || null,
        }])

      if (profileError) throw profileError

      // Update tryout status to accepted
    ...
    <form onSubmit={handleSubmit} className="space-y-8">
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            type="button"
            onClick={handlePromoteToPlayer}
            className="flex items-center gap-2 px-4 py-2 bg-green-600/20 hover:bg-green-600/30 text-green-400 border border-green-600/30 rounded-lg transition-all"
          >
            <UserPlus className="w-4 h-4" />
            <span className="hidden sm:inline">{t('promoteToPlayer')}</span>
          </button>
          
          <button
            type="button"
            onClick={handleDelete}
            className="flex items-center gap-2 px-4 py-2 bg-red-600/20 hover:bg-red-600/30 text-red-400 border border-red-600/30 rounded-lg transition-all"
          >
      ...
      {/* Title */}
      <div>
        <h1 className="text-2xl font-bold text-white">{tForm('editScoutProfile')}</h1>
        <p className="text-gray-400 text-sm mt-1">{tForm('editScoutDesc')}</p>
      </div>
      ...
      <div className="flex justify-between gap-4 pt-4">
        ...
        >
          {loading ? tForm('updatingProfile') : tForm('updateProfile')}
        </button>
      </div>
    </form>
  )

...
export default function EditMatchClient({ matchId, teamId }: EditMatchClientProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(`/dashboard/admin/teams/view/${teamId}/matches/${matchId}`)
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Link href={`/dashboard/admin/teams/view/${teamId}/matches/${matchId}`}>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {/* Player Statistics - Reusing same structure from Add */}
        <div className="bg-dark-card border border-gray-800 rounded-lg p-6">

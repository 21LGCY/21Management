...
import { Globe, Users, TrendingUp, Gamepad2 } from 'lucide-react'
...
export default function ZonesInterfaceManager({ teamId, team, teamCategory }: ZonesInterfaceManagerProps) {
  ...
  // Get current zones based on game type
  const currentZones = isCS2Team ? CS2_ZONES : VALORANT_ZONES
  ...
  const fetchTryouts = async () => {
    setLoading(true)
    try {
      // For CS2 teams, show all CS2 tryouts
  ...
  const getZoneColor = (zone: string): string => {
    ...
    const colors = isCS2Team ? cs2Colors : valorantColors
    ...
    <div className="space-y-6">
      ...
      <div className="bg-dark-card border border-gray-800 rounded-lg p-6">
        <h3 className="text-xl font-bold text-white mb-4">
          {isCS2Team ? t('cs2ZoneDefinitions') || 'CS2 Zone Definitions' : t('valorantZoneDefinitions') || 'VALORANT Zone Definitions'}
        </h3>
